...
function mwColor(mw) {
  if (mw < 0) return 'text-red-500';
  if (mw > 0) return 'text-green-500';
...
function exportToCSV(intervals) {
  let csv = 'Interval #,Timestamp,LMP ($/MWh),Power MW,Energy MWh,Cost/Rev ($),Running Total ($),Notes\n';
  intervals.forEach(i => {
    const ts = i.timestamp ? new Date(i.timestamp).toLocaleString() : '';
    csv += `${i.interval_num},"${ts}",${(i.lmp||0).toFixed(2)},${i.power_mw},${(i.energy_mwh||0).toFixed(4)},${(i.cost_revenue||0).toFixed(2)},${(i.running_total||0).toFixed(2)},"${(i.notes||'').replace(/"/g,'""')}"\n`;
  });
...
export default function IntervalTable({ intervals, onAdd, onEdit, onDelete }) {
        ...
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              {intervals?.length > 0 && (
                <Button size="sm" variant="outline" className="h-8 text-xs" onClick={() => exportToCSV(intervals)}>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <SortHeader label="Cost/Rev ($)" field="cost_revenue" />
              ...
              <TableBody>
                {sorted.length === 0 ? (
                  ...
                  const mw = i.power_mw ?? 0;
                  const costRevSign = mw < 0 ? '-' : '';
                  return (
                    <TableRow key={i.interval_num} className="text-xs">
                      ...
                      <TableCell className={`font-mono font-medium ${mwColor(mw)}`}>{mw}</TableCell>
                      <TableCell className="font-mono">{(i.energy_mwh || 0).toFixed(4)}</TableCell>
                      <TableCell className={`font-mono font-medium ${mwColor(mw)}`}>
                        {costRevSign}${Math.abs(i.cost_revenue || 0).toFixed(2)}
                      </TableCell>
                      <TableCell className={`font-mono font-medium ${totalColor(i.running_total)}`}>
                        {i.running_total < 0 ? '-' : ''}${Math.abs(i.running_total || 0).toFixed(2)}
                      </TableCell>
